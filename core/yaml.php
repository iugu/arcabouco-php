...
class yaml {
  ...
  */
  ...
  public static function dump($array,$indent = false,$wordwrap = false) {
    $spyc = new yaml;
    return $spyc->_dump($array,$indent,$wordwrap);
  }
  ...
  /**
     ...
     * @param int $wordwrap Pass in 0 for no wordwrap, false for default (40)
     */
  public function _dump($array,$indent = false,$wordwrap = false) {
...
/* LOADING FUNCTIONS */
  ...
  private function _load($input) {
    ...
    for ($i = 0; $i < count($Source); $i++) {
      ...
      if ($literalBlockStyle = $this->startsLiteralBlock($line)) {
        $line = rtrim ($line, $literalBlockStyle . "\r");
        $literalBlock = '';
        $line .= $this->LiteralPlaceHolder;

        while (isset($Source[$i+1]) && $this->literalBlockContinues($Source[++$i], $lineIndent)) {
  ...
  private function _inlineEscape($inline) {
    ...
    // Check for sequences
    if (preg_match_all('/\[(.+)\]/Um',$inline,$seqs)) {
      $inline = preg_replace('/\[(.+)\]/U','YAMLSeq',$inline);
      $seqs   = $seqs[0];
    }

    // Check for mappings
    if (preg_match_all('/{(.+)}/Um',$inline,$maps)) {
  ...
  private function addArray ($array, $indent) {
    ...
    $tempPath = yaml::flatten ($this->path);
    ...
    if ($this->_containsGroupAlias) {
      do {
        ...
        eval ('$value = $this->result' . yaml::flatten ($groupPath) . ';');
      } while (false);
